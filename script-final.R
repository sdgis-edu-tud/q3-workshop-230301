...
# Show the population trend of all cities in the region
pop_13_22_long |>
  # filter(cities %in% c("BE001F", "DE001F", "NL001F", "UK001F")) |> 
  filter(substr(cities, 3, 6) == "001F") |> 
  group_by(cities) |> 
  mutate(label = if_else(TIME_PERIOD == max(TIME_PERIOD), as.character(cities), NA_character_)) |> 
  ungroup() |> 
  ...
  labs(title = "Number of inhabitants in the region") +
